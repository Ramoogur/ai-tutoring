...
const Time = ({ topic, user, navigateTo }) => {
  ...
  const getTimeOptions = (question) => {
    if (!question) return [];
    if (question.options) return question.options;
    if (question.images) return question.images.map(img => img.description || img.id);
    if (question.items) return question.items.map(item => item.label || item.text);
    return [];
  };
        ...
        <div className="question-header">
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            <TTSButton 
              ...
              options={getTimeOptions(isFrench && translatedQuestions[currentQuestionIndex] ? translatedQuestions[currentQuestionIndex] : currentQuestion)}
            />
            <TranslationButton 
              onToggle={async () => {
                ...
                try {
                  if (isFrench) {
                    ...
                    const uiTexts = { 'Time Quiz': 'Quiz de Temps', 'Question': 'Question', 'of': 'de', 'Checking...': 'Vérification...', 'Next Question': 'Question Suivante', 'Quiz Complete!': 'Quiz Terminé!', 'Back to Dashboard': 'Retour au Tableau de Bord' };
